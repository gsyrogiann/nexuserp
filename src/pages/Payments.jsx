...
const formFields = [
  { key: 'number', label: 'Payment Number', placeholder: 'e.g. PAY-001' },
  {
    ...
    type: 'select',
    options: [
      { value: 'incoming', label: 'Incoming (Customer)' },
      { value: 'outgoing', label: 'Outgoing (Supplier)' }
    ]
  },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatsCard
          ...
          value={`€${incoming.toLocaleString('el-GR', { minimumFractionDigits: 2 })}`}
          icon={ArrowDownLeft}
        />
        <StatsCard
          label="Total Outgoing"
          value={`€${outgoing.toLocaleString('el-GR', { minimumFractionDigits: 2 })}`}
          icon={ArrowUpRight}
        />
        <StatsCard
          label="Net Cash Flow"
          value={`€${(incoming - outgoing).toLocaleString('el-GR', { minimumFractionDigits: 2 })}`}
          icon={CreditCard}
          trend={incoming >= outgoing ? 'up' : 'down'}
        />
      </div>
      ...
      <EntityFormDialog
        ...
        title={editing?.id ? 'Edit Payment' : 'New Payment'}
        fields={formFields}
        initialData={editing}
