...
export default function Quotes() {
  ...
  const { data: quotes } = useQuery({ queryKey: ['quotes'], queryFn: () => base44.entities.Quote.list(), initialData: [] });
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Quotes" subtitle={`${quotes.length} quotes`} actionLabel="New Quote" onAction={() => { setEditing({}); setDialogOpen(true); }} />
      ...
      <DataTable columns={columns} data={quotes} onRowClick={(row) => { setEditing(row); setDialogOpen(true); }} />
      <DocumentFormDialog open={dialogOpen} onOpenChange={setDialogOpen} title={editing?.id ? 'Edit Quote' : 'New Quote'} initialData={editing} onSubmit={handleSubmit} customers={customers} products={products} entityType="customer" />
    </div>
  );
}